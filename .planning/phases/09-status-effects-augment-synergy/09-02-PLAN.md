...
<task type="tdd">
  ...
  <action>
    ...
    **Event subscription strategy *(Review: Event Buffering Complexity — MEDIUM concern resolved)*:**

    The EventBus is **queued** — `emit()` pushes to a queue, `flush()` processes all queued events in order. During a flush, handlers that emit new events are processed in subsequent iterations of the same flush cycle (up to `MAX_FLUSH_DEPTH=10`). This means ALL cascading events from a single action (FIRMWARE_ACTIVATED → DAMAGE_DEALT → ENTITY_DIED) resolve within one `flush()` call.

    **Concrete approach — collect-then-evaluate:**
    1. In `init()`, subscribe to `FIRMWARE_ACTIVATED`, `DAMAGE_DEALT`, and `ENTITY_DIED` events.
    2. Each handler **only collects data** into a mutable `pendingContext: TriggerContext | null` buffer — no trigger evaluation happens inline.
       - `FIRMWARE_ACTIVATED` handler: creates a fresh `pendingContext = { firmwareActivated: true, damageDealt: 0, killCount: 0, ... }` and reads Heat/HP from world state.
       - `DAMAGE_DEALT` handler: if `pendingContext` is not null, adds `damageDealt += event.amount`.
       - `ENTITY_DIED` handler: if `pendingContext` is not null, increments `killCount++`.
    3. Subscribe to the `PLAYER_ACTION` event (which fires AFTER all system processing in the playerActionHandler). In this handler, if `pendingContext` is not null, call `processTriggersForEntity(entityId, pendingContext)` and then set `pendingContext = null`.

    This avoids `setTimeout(0)` or microtask tricks entirely. The `PLAYER_ACTION` event is already emitted at the end of every player action in `engine-factory.ts` (line ~173: `eventBus.emit('PLAYER_ACTION', { action, entityId })`), making it the natural "action complete" signal.

    **TriggerContext clearing *(Review suggestion)*:** `pendingContext` is set to `null` after `processTriggersForEntity()` completes and also at the start of `resetTurnState()`. This prevents trigger leaking across turns.

    **Circular Augment protection *(Review: Infinite Loop Protection — LOW concern)*:**
    Add a private `isResolving: boolean = false` guard on the AugmentSystem. Set `isResolving = true` before calling `processTriggersForEntity()`, reset to `false` after. All event handlers check `isResolving` and skip collection if true. This prevents augment payloads (which may emit events like `STATUS_EFFECT_APPLIED` or `HEAT_CHANGED`) from re-triggering the augment evaluation loop.
    ...
    - `isResolving` guard prevents circular re-triggering (Review: Infinite Loop Protection)
    - `pendingContext` is null after `processTriggersForEntity()` completes (Review: Context Clear Logic)
  </action>
  <acceptance_criteria>
    ...
    - `grep -q "isResolving" src/game/systems/augment.ts` exits 0
    - `grep -q "pendingContext" src/game/systems/augment.ts` exits 0
...
- isResolving guard prevents circular augment cascades (Review: Infinite Loop Protection)
- pendingContext cleared after evaluation to prevent trigger leaking (Review: Context Clear Logic)
- Collect-then-evaluate pattern uses PLAYER_ACTION as action-complete signal (Review: Event Buffering)
</must_haves>
