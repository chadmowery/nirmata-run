...
<task type="tdd">
  ...
  <action>
    ...
    3. **Add `getTotalMagnitude(entityId, effectName): number`** — Returns the sum of all active magnitudes for a given effect name. Forward-compat for future "Total Poison"-style mechanics where stacking sums matter. *(Review suggestion: Gemini recommended adding this now to avoid a second refactor later.)*

    4. **Refactor tick timing (D-03):** Remove the `TURN_START` event subscription from `init()`. The `tickDown(entityId)` method remains unchanged but is now called externally from the TurnManager handlers (engine-factory.ts) at the start of each entity's turn, not globally. This ensures a 1-turn debuff lasts exactly one of the afflicted entity's actions.

    5. **Update `applyEffect()` to emit `STATUS_EFFECT_APPLIED`:**
       ```typescript
    ...
    6. **Update `tickDown()` — explicit effect removal and expiry events** *(Review concern: Status Effect Cleanup)*:
       - Decrement `duration` by 1 for each effect in the array
       - **Filter out expired effects** where `duration <= 0` using `effects.filter(e => e.duration > 0)` and write the filtered array back to the component
       - For each expired effect (before removal), emit `STATUS_EFFECT_EXPIRED`:
       ```typescript
       ...
       ```
       - Also emit `MESSAGE_EMITTED` for each expired effect: `"{effectName} has worn off."`
       This is how the Phase 8 stub already works, but it is now explicitly documented per review feedback.

    7. **Keep the existing `init()` and `dispose()` methods** but they become no-ops (empty body). Do NOT subscribe to `TURN_START` anymore. The system is now externally driven.
    ...
    - Test `getTotalMagnitude()` returns sum of all magnitudes
    - Test `getTotalMagnitude()` returns 0 when no effect exists
  ...
  <acceptance_criteria>
    ...
    - `grep -q "getTotalMagnitude" src/game/systems/status-effects.ts` exits 0
    ...
    - `grep -q "filter" src/game/systems/status-effects.ts` exits 0
...
- getTotalMagnitude() returns sum of all magnitudes for forward-compat (Review suggestion)
- tickDown() is called per-entity (D-03), not globally via TURN_START
- tickDown() explicitly filters out expired effects from the array (Review: Status Effect Cleanup)
